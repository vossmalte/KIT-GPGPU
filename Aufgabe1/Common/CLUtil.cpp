...
{
	// replace with correct work group sizing code here (Sec. 4.6)
...
void CLUtil::PrintBuildLog(cl_program Program, cl_device_id Device)
{
	...
	// nah, I like it better without any output
	if (buildStatus != CL_SUCCESS) {
...
{
	// Measure average execution time of the kernel
	CTimer timer;
	cl_int clErr;

	// wait until the command queue is empty... inefficient but allows accurate timing
	...
	if (clErr != CL_SUCCESS) {
		cout << "kernel execution failure: " << GetCLErrorString(clErr) << endl;
		return -1;
	}
	double ms = timer.GetElapsedMilliseconds() / double(NIterations);

	cout << "Average execution time of the kernel: " << ms << "ms." << endl;
